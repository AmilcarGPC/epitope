            ...
            if chain_pdb == -1 or chain._id == chain_pdb:
                ...
                for residue in chain:
                    if residue.get_resname() != "HOH" and residue._id[0] == " ":  # Exclude water molecules
                        ...
                        if res_name in amino_acid_dict:
                            sequence += amino_acid_dict[res_name]
                        else:
                            # Puedes agregar un mensaje de advertencia o simplemente saltarlo
                            print(f"Residuo desconocido: {res_name}")
                            continue
                        amino_acid += 1
                        for atom in residue:
                            if atom.get_name() == 'CA':

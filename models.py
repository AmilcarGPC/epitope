...
def predict_model(test_batches, kidera_embedding, params):
    ...
    # Cargar el modelo con map_location para manejar dispositivos CUDA
...
def plot_loss(train_loss_list, val_loss_list, test_loss_list, epochs, save_dir, metric_type, description):
    """
    Genera y guarda una gráfica de la pérdida (o cualquier otra métrica) en función de las épocas.
    
    Parámetros:
      - train_loss_list: Lista de pérdidas en entrenamiento.
      - val_loss_list: Lista de pérdidas en validación.
      - test_loss_list: Lista de pérdidas en test (opcional, se grafica si existe).
      - epochs: Número total de épocas.
      - save_dir: Directorio donde se guardará la imagen.
      - metric_type: Tipo de métrica (por ejemplo, 'loss').
      - description: Descripción para identificar el modelo en la gráfica.
    """
    import matplotlib.pyplot as plt

    epochs_range = range(1, epochs + 1)
    plt.figure()
    plt.plot(epochs_range, train_loss_list, label='Pérdida entrenamiento')
    plt.plot(epochs_range, val_loss_list, label='Pérdida validación')
    if test_loss_list and len(test_loss_list) > 0:
        plt.plot(epochs_range, test_loss_list, label='Pérdida test')
    plt.xlabel('Épocas')
    plt.ylabel(metric_type.capitalize())
    plt.title(f'{metric_type.capitalize()} vs Épocas - {description}')
    plt.legend()
    plt.savefig(f"{save_dir}/{metric_type}_plot_{description}.png")
    plt.close()
